...
public interface IBlockChainIndex
{
    ...
    /// <inheritdoc cref="GetBlockHashesByOffset"/>
    IAsyncEnumerable<(long Index, BlockHash Hash)> GetBlockHashesByOffsetAsync(
        int? offset = null, int? limit = null, bool desc = false, Address? miner = null);
    ...
    /// <inheritdoc cref="GetInvolvedTxIdsByAddress"/>
    IAsyncEnumerable<TxId> GetInvolvedTxIdsByAddressAsync(
        Address address, int? offset = null, int? limit = null, bool desc = false);
    ...
    internal void SynchronizeForever<T>(BlockChain<T> chain, CancellationToken stoppingToken)
        where T : IAction, new();

    internal Task SynchronizeForeverAsync<T>(BlockChain<T> chain, CancellationToken stoppingToken)
        where T : IAction, new();

    internal void Synchronize<T>(IStore store, CancellationToken stoppingToken)
        where T : IAction, new();
