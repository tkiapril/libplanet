...
public abstract class BlockChainIndexBase : IBlockChainIndex
{
    ...
    void IBlockChainIndex.SynchronizeForever<T>(
        BlockChain<T> chain, CancellationToken stoppingToken)
    {
        var newBlockEncountered = new AsyncAutoResetEvent();
        chain.TipChanged += (_, _) => newBlockEncountered.Set();
        while (true)
        {
            ((IBlockChainIndex)this).Synchronize<T>(chain.Store, stoppingToken);
            newBlockEncountered.Wait(stoppingToken);
        ...
        BlockChain<T> chain, CancellationToken stoppingToken)
    {
        ...
        while (true)
        {
            await ((IBlockChainIndex)this).SynchronizeAsync<T>(chain.Store, stoppingToken);
            await newBlockEncountered.WaitAsync(stoppingToken);
            if (stoppingToken.IsCancellationRequested)
            {
                return;
            }
    ...
    void IBlockChainIndex.Synchronize<T>(IStore store, CancellationToken stoppingToken)
    {
        ...
        if (indexTipIndex >= 0)
        {
            var indexHash = IndexToBlockHash(0);
        ...
        while (indexEnumerator.MoveNext() && indexTipIndex + processedBlockCount < chainTipIndex)
        {
            ...
            IndexImpl(blockDigest, txs, addBlockContext, stoppingToken);
        ...
        CommitIndexingContext(addBlockContext);
